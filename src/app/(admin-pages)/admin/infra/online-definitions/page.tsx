...
export default function OnlineDefinitionsPage() {
  ...
  const saveDraft = async () => {
    ...
    setBusy(true); setMessage("")
    try {
      const response = await request.put<OnlineDefinitionDetail>(`${endpoint}/${detail.code}/draft`, { expectedLockVersion: lockVersion, model: JSON.parse(modelText), interaction: JSON.parse(interactionText) })
      if (!response.success || !response.data) { setMessage(response.error ?? "Draft 保存失败"); return }
      setDetail(response.data); setMessage("Draft 已保存；字段投影已同步，需重新校验后才能发布")
