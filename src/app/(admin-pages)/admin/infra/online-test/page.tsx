...
import type { OnlineRuntimeRecord, OnlineRuntimeRecordPage, OnlineTestSessionDetail } from "@/modules/online/backend/application/online-runtime.contract"
...
export default function OnlineTestPage() {
  ...
  const loadRecords = async (active = session) => {
    if (!active) return
    const response = await request.get<OnlineRuntimeRecordPage>(`${definitionsEndpoint}/${active.definitionCode}/test-sessions/${active.id}/records`, { page: 1, pageSize: 50 })
  ...
  const start = async () => {
    ...
    try {
      ...
      setSession(response.data); setRecords(null); setForm({}); setEditing(null); await loadRecords(response.data)
  ...
  const remove = async (record: OnlineRuntimeRecord) => {
    if (!session || !confirm("确认删除此 sandbox 测试记录？")) return
    setBusy(true)
    try {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-6">
    ...
    <section className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4"><label className="min-w-72 flex-1 text-sm"><span className="mb-1 block font-medium">已发布单表 Definition</span><select value={code} onChange={(event) => setCode(event.target.value)} className="h-9 w-full rounded border px-3"><option value="">请选择</option>{published.map((item) => <option key={item.id} value={item.code}>{item.name} · {item.code}</option>)}</select></label><button disabled={busy || !code} onClick={() => void start()} className="h-9 rounded bg-blue-600 px-4 text-sm font-medium text-white disabled:opacity-50">开始 Online Test</button></section>
    ...
    {session.runtime.views[0] && <section className="overflow-hidden rounded-lg border bg-slate-50 p-4"><h2 className="mb-3 text-sm font-semibold">已发布 Release 视图预览 · {session.runtime.views[0].code}</h2><Render config={onlinePuckConfig} data={session.runtime.views[0].puckData as Data} /></section>}
    <section className="rounded-lg border bg-white p-4"><div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">{editing ? "编辑测试记录" : "新增测试记录"}</h2>{editing && <button onClick={() => { setEditing(null); setForm({}) }} className="text-sm text-slate-600">取消编辑</button>}</div><div className="grid gap-3 md:grid-cols-2">{fields.map((field) => { const config = interaction.get(field.code); const disabled = config?.readOnly; const value = form[field.code]; return <label key={field.code} className="text-sm"><span className="mb-1 block">{config?.label ?? field.code}{!field.nullable && " *"}</span>{field.type === "boolean" ? <input disabled={disabled} checked={Boolean(value)} onChange={(event) => setValue(field, event.target.checked)} type="checkbox" className="h-4 w-4" /> : <input disabled={disabled} type={field.type === "integer" || field.type === "decimal" ? "number" : field.type === "date" ? "date" : field.type === "datetime" ? "datetime-local" : "text"} value={typeof value === "string" || typeof value === "number" ? value : ""} onChange={(event) => setValue(field, event.target.value)} className="h-9 w-full rounded border px-3 disabled:bg-slate-100" />}</label>})}</div><button disabled={busy} onClick={() => void submit()} className="mt-4 rounded bg-emerald-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{editing ? "保存修改" : "创建记录"}</button></section>
    <section className="overflow-hidden rounded-lg border bg-white"><div className="flex items-center justify-between border-b p-4"><h2 className="font-semibold">Sandbox 记录（{records?.total ?? 0}）</h2><button onClick={() => void loadRecords()} className="text-sm text-blue-700">刷新</button></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b bg-slate-50 text-xs text-slate-500"><tr>{fields.map((field) => <th key={field.code} className="p-3">{interaction.get(field.code)?.label ?? field.code}</th>)}<th className="p-3 text-right">操作</th></tr></thead><tbody>{records?.items.length ? records.items.map((record) => <tr key={record.id} className="border-b last:border-0">{fields.map((field) => <td key={field.code} className="p-3">{String(record.data[field.code] ?? "-")}</td>)}<td className="p-3 text-right"><button onClick={() => { setEditing(record); setForm(record.data) }} className="mr-3 text-blue-700">编辑</button><button onClick={() => void remove(record)} className="text-red-700">删除</button></td></tr>) : <tr><td colSpan={fields.length + 1} className="p-8 text-center text-slate-500">暂无测试记录</td></tr>}</tbody></table></div></section></>}
  </main>
}
