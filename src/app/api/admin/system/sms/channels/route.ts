...
import { createSmsChannelSchema, systemModulePageQuerySchema } from "@/backend/validators/system-module.validator"
import { SystemSmsService } from "@/backend/services/system-sms.service"
import { PERMISSIONS } from "@/backend/constants/permissions"
import { ensurePermission } from "@/backend/lib/permission-guard"

export async function GET(request: Request) {
  try {
    ensurePermission(request, PERMISSIONS.SYSTEM_SMS_CHANNEL_VIEW)
...
export async function POST(request: Request) {
  try {
    const auth = ensurePermission(request, PERMISSIONS.SYSTEM_SMS_CHANNEL_CREATE)
