...
export async function GET(request: Request) {
  try {
    await requirePlatformAdmin(request, PERMISSIONS.SYSTEM_OAUTH2_CLIENT_VIEW)
    ...
    const data = await SystemOauth2Service.listClients(input)
    return NextResponse.json({ success: true, data })
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error?.message ?? "查询失败" }, { status: getAuthErrorStatus(error) })
  }
}
