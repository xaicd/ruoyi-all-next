...
export const GET = withAdminRoute(async (request) => {
  try {
    const input = parseActionQuery(SYSTEM_ACTION_SCHEMAS["system.listPosts"], request)
...
export const POST = withAdminRoute(async (request) => {
  try {
    const input = parseActionBody(SYSTEM_ACTION_SCHEMAS["system.createPost"], await request.json())
