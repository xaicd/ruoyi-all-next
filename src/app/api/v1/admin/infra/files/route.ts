...
export async function GET(request: Request) {
  try {
    ensurePermission(request, PERMISSIONS.INFRA_FILE_VIEW)
...
export async function POST(request: Request) {
  try {
    ...
    const input = z.object({ configId: z.string().trim().min(1), name: z.string().trim().optional(), path: z.string().trim().min(1), url: z.string().trim().min(1), type: z.string().trim().optional(), size: z.coerce.number().int().min(0) }).parse(body)
