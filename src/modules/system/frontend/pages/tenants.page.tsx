      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-sm">
            <thead><tr className="border-b border-slate-100 bg-slate-50/80 text-left text-xs font-medium text-slate-500"><th className="px-5 py-3">租户名称</th><th className="px-4 py-3">租户编码</th><th className="px-4 py-3">联系人</th><th className="px-4 py-3">联系电话</th><th className="px-4 py-3">套餐</th><th className="px-4 py-3 text-center">账号额度</th><th className="px-4 py-3">过期时间</th><th className="px-4 py-3">状态</th><th className="px-5 py-3 text-right">操作</th></tr></thead>
            <tbody>{loading ? <tr><td colSpan={9} className="px-4 py-14 text-center text-slate-400">加载中...</td></tr> : data.items.length === 0 ? <tr><td colSpan={9} className="px-4 py-14 text-center text-slate-400">暂无数据</td></tr> : data.items.map((t) => <tr key={t.id} className="border-b border-slate-100 last:border-0 transition hover:bg-blue-50/30"><td className="px-5 py-3.5 font-medium text-slate-800">{t.name}</td><td className="px-4 py-3.5"><code className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-600">{t.tenantCode}</code></td><td className="px-4 py-3.5 text-slate-700">{t.contactName || "-"}</td><td className="px-4 py-3.5 text-slate-500">{t.contactPhone || "-"}</td><td className="px-4 py-3.5"><span className="rounded bg-blue-50 px-2 py-1 text-xs text-blue-700">{t.packageName || "未分配"}</span></td><td className="px-4 py-3.5 text-center font-medium text-slate-700">{t.accountCount}</td><td className="px-4 py-3.5 text-slate-500">{t.expireTime ? new Date(t.expireTime).toLocaleDateString("zh-CN") : "-"}</td><td className="px-4 py-3.5"><button onClick={() => handleToggleStatus(t)} className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium transition ${t.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700 hover:bg-emerald-100" : "bg-red-50 text-red-700 hover:bg-red-100"}`}>{t.status === "ACTIVE" ? "启用" : "禁用"}</button></td><td className="px-5 py-3.5 text-right"><div className="inline-flex gap-3 text-sm"><button onClick={() => handleAssignPackage(t)} className="text-violet-600 hover:text-violet-800">套餐</button><button onClick={() => handleEdit(t)} className="text-blue-600 hover:text-blue-800">编辑</button><button onClick={() => handleDelete(t)} className="text-red-500 hover:text-red-700">删除</button></div></td></tr>)}</tbody>
          </table>
        </div>
...
function TenantFormDialog({ tenant, onSubmit, onClose }: { tenant: Tenant | null; onSubmit: (data: Record<string, any>) => void; onClose: () => void }) {
  const [form, setForm] = useState({
    ...
    packageId: tenant?.packageId ?? "", status: tenant?.status ?? "ACTIVE", expireTime: tenant?.expireTime ? tenant.expireTime.split("T")[0] : "",
    accountCount: tenant?.accountCount ?? 1, adminUsername: "", adminNickname: "", adminPassword: "", adminPhone: "", adminEmail: "",
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (payload.expireTime) payload.expireTime = new Date(payload.expireTime).toISOString(); else delete payload.expireTime
      ...
      <div role="dialog" aria-modal="true" className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <div className="space-y-4 overflow-y-auto px-6 py-5">
            ...
            <Section title="服务与套餐" description="套餐决定租户可授权菜单；账号额度限制可创建的有效用户数量。">
              <div className="grid gap-4 md:grid-cols-3">
                <div><Label required>账号额度</Label><input type="number" min={1} value={form.accountCount} onChange={(e) => update("accountCount", Number(e.target.value))} className={fieldClass} /></div>
            ...
            {!tenant && <Section title="首个租户管理员" description="创建后使用该账号、密码及上方租户编码登录租户控制台。">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div><Label required>管理员账号</Label><input required value={form.adminUsername} onChange={(e) => update("adminUsername", e.target.value)} placeholder="tenant_admin" className={fieldClass} /></div>
