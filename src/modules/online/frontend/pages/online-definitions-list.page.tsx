...
export default function OnlineDefinitionsListPage() {
  ...
  return <main className="min-h-full bg-slate-50 p-4 lg:p-6"><div className="mx-auto max-w-[1500px] space-y-4">
    <header className="rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-semibold text-blue-600">ONLINE LOW-CODE · BUSINESS MODELS</div><h1 className="mt-1 text-xl font-semibold text-slate-900">业务建模</h1><p className="mt-1 text-sm text-slate-500">新业务在此完成设计、校验、计划、审批、应用 DDL 和发布；发布后可进行功能测试并批量生成 RuoYi CRUD 代码。</p></div><div className="flex gap-2"><button type="button" disabled={batchGenerating || !selectedCodes.length} onClick={() => void downloadGeneratedCode(selectedCodes)} className="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-medium text-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">{batchGenerating ? "生成中…" : "生成"}</button><Link href="/admin/infra/online-definitions/new" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">+ 新建</Link></div></div></header>
