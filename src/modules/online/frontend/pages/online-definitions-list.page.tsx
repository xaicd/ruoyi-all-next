...
import type { OnlineDefinitionDetail, OnlineDefinitionPage, OnlineDefinitionSummary, OnlineModelType } from "@/modules/online/backend/application/online-definition.contract"
...
type SchemaPlan = { id: string; risk: string; status: "REVIEW_REQUIRED" | "APPROVED" | "APPLIED" | "FAILED" | "SUPERSEDED" | "DRAFT" }
...
export default function OnlineDefinitionsListPage() {
  ...
  const syncDatabase = async (item: OnlineDefinitionSummary) => {
    setMoreMenu(null)
    if (!confirm(`确认同步「${item.name}」的真实数据库表？\n系统将校验模型、审批并应用受控 DDL，创建或变更物理表。`)) return
    setOperatingCode(item.code); setMessage("")
    try {
      const detail = await request.get<OnlineDefinitionDetail>(`${endpoint}/${item.code}`)
      if (!detail.success || !detail.data) throw new Error(detail.error ?? "无法读取业务模型")
      const storage = (detail.data.revision?.model as { storage?: { kind?: string } } | undefined)?.storage?.kind
      if (storage !== "MANAGED_TABLE") throw new Error("当前模型为沙箱记录，不会创建物理表；请新建或调整为真实物理表模型")
      const validation = await request.post<OnlineDefinitionDetail>(`${endpoint}/${item.code}/validate`, { expectedLockVersion: detail.data.lockVersion })
      if (!validation.success || !validation.data) throw new Error(validation.error ?? "模型校验失败")
      const created = await request.post<SchemaPlan>(`${endpoint}/${item.code}/schema-plans`, { expectedLockVersion: validation.data.lockVersion })
      if (!created.success || !created.data) throw new Error(created.error ?? "Schema 计划生成失败")
      let plan = created.data
      let current = await request.get<OnlineDefinitionDetail>(`${endpoint}/${item.code}`)
      if (!current.success || !current.data) throw new Error(current.error ?? "无法刷新 Schema 计划状态")
      if (plan.status === "REVIEW_REQUIRED") {
        const approved = await request.post<SchemaPlan>(`${endpoint}/${item.code}/schema-plans/${plan.id}/approve`, { expectedLockVersion: current.data.lockVersion, note: "确认同步真实物理表" })
        if (!approved.success || !approved.data) throw new Error(approved.error ?? "Schema 计划审批失败")
        plan = approved.data
        current = await request.get<OnlineDefinitionDetail>(`${endpoint}/${item.code}`)
        if (!current.success || !current.data) throw new Error(current.error ?? "无法刷新审批状态")
      }
      if (plan.status !== "APPROVED") throw new Error(`当前 Schema Plan 状态为 ${plan.status}，不能应用物理表变更`)
      const applied = await request.post<SchemaPlan>(`${endpoint}/${item.code}/schema-plans/${plan.id}/apply`, { expectedLockVersion: current.data.lockVersion, confirmation: "APPLY_MANAGED_TABLE" })
      if (!applied.success) throw new Error(applied.error ?? "物理表应用失败")
      await loadPage(); setMessageTone("success"); setMessage("数据库已真实同步：物理表已创建或变更；现在可发布 Release")
    } catch (error) {
      await loadPage(); setMessageTone("error"); setMessage(error instanceof Error ? error.message : "数据库同步失败")
    } finally { setOperatingCode(null) }
  }
  ...
  return <main className="min-h-full bg-slate-50 p-4 lg:p-6"><div className="mx-auto max-w-[1500px] space-y-4">
    <header className="rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-semibold text-blue-600">ONLINE LOW-CODE · BUSINESS MODELS</div><h1 className="mt-1 text-xl font-semibold text-slate-900">业务模型</h1><p className="mt-1 text-sm text-slate-500">业务模型用于在线设计真实物理表与页面；同步数据库会受控执行 DDL，发布 Release 后可生成 RuoYi CRUD 代码。</p></div><div className="flex gap-2"><button type="button" disabled={batchGenerating || !selectedCodes.length} onClick={() => void downloadGeneratedCode(selectedCodes)} className="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-medium text-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">{batchGenerating ? "生成中…" : "生成"}</button><Link href="/admin/infra/online-test" className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">AUTO报表</Link><Link href="/admin/infra/online-definitions/new" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">+ 新建业务模型</Link></div></div></header>
    ...
    {moreMenu && typeof document !== "undefined" && createPortal(<><button aria-label="关闭更多操作" className="fixed inset-0 z-40 cursor-default" onClick={() => setMoreMenu(null)} /><div role="menu" className="fixed z-50 w-36 rounded-lg border border-slate-200 bg-white py-1 text-sm shadow-xl" style={{ left: moreMenu.left, top: moreMenu.top }}>{moreMenu.item.publishedReleaseId ? <Link role="menuitem" onClick={() => setMoreMenu(null)} href={`/admin/infra/online-test?definition=${encodeURIComponent(moreMenu.item.code)}`} className="block px-3 py-2 hover:bg-slate-50">AUTO报表</Link> : <span className="block cursor-not-allowed px-3 py-2 text-slate-400" title="请先发布版本">AUTO报表（需发布）</span>}<button role="menuitem" disabled={busy(moreMenu.item)} onClick={() => void syncDatabase(moreMenu.item)} className="block w-full px-3 py-2 text-left hover:bg-slate-50 disabled:opacity-50">同步</button><div className="my-1 border-t border-slate-100"/><button role="menuitem" onClick={() => requestConfirmation(moreMenu.item, "archive")} className="block w-full px-3 py-2 text-left hover:bg-slate-50">移除</button>{!moreMenu.item.publishedReleaseId && <button role="menuitem" onClick={() => requestConfirmation(moreMenu.item, "delete")} className="block w-full px-3 py-2 text-left text-rose-600 hover:bg-rose-50">删除</button>}</div></>, document.body)}
