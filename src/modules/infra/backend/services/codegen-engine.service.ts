...
export type CodegenTemplate =
  ...
  | "MASTER_DETAIL"  // Online 主子表（多个子定义）
  | "WORKFLOW"       // 工作流审批
  | "SINGLETON"      // 单例配置页
...
export type CodegenAdvancedField = ColumnInfo & {
  ...
  dictionaryCode?: string; relation?: { code: string; targetDefinitionCode: string; targetReleaseId?: string; targetField: string; onDelete: "RESTRICT" | "SET_NULL" }
}
export type CodegenMasterDetailChild = { code: string; targetDefinitionCode: string; targetReleaseId: string; foreignKeyField: string; display: "TABLE" | "TABS"; fields: CodegenAdvancedField[] }
export type CodegenAdvancedConfig = {
  fields: CodegenAdvancedField[]
  model?: { type: "SINGLE" | "TREE" | "MASTER_DETAIL"; tree?: { parentField: string; sortField?: string; rootValue?: string | number | null }; masterDetail?: { children: CodegenMasterDetailChild[] } }
...
export type CodegenConfig = {
  ...
  /** 父菜单 ID（用于菜单注册） */
  parentMenuId?: string
  /** Generated feature permission prefix, e.g. system:notice. Defaults to <module>:<resource>. */
...
function generateRoute(config: CodegenConfig): CodegenOutput {
  ...
  if (searchParams.get("tree") === "true") return NextResponse.json({ success: true, data: await ${className}Service.tree(input) })` : ""
...
export const GET = withAdminRoute(async (request: Request) => {
  ...
  if (id) return NextResponse.json({ success: true, data: await ${className}Service.get(id) })
  const input = ${camel}PageQuerySchema.parse(Object.fromEntries(Array.from(searchParams.keys()).map((key) => {
...
export const POST = withAdminRoute(async (request: Request) => {
  const input = ${camel}CreateSchema.parse(await request.json())
...
export const PUT = withAdminRoute(async (request: Request) => {
  const input = ${camel}UpdateSchema.parse(await request.json())
...
export const DELETE = withAdminRoute(async (request: Request) => {
  ...
  return NextResponse.json({ success: true, data: await ${className}Service.delete(id) })
...
export const GET = withAdminRoute(async (request: Request) => {
  const page = await ${className}Service.page({ page: 1, pageSize: 10000, ...Object.fromEntries(new URL(request.url).searchParams) })
...
import { ${camel}CreateSchema } from "@/modules/${moduleName}${sub}/backend/validators/${kebab}.validator"
...
export const POST = withAdminRoute(async (request: Request) => {
  ...
  for (let index = 0; index < body.rows.length; index++) { try { await ${className}Service.create(${camel}CreateSchema.parse(body.rows[index])); imported++ } catch (error) { errors.push({ index, error: error instanceof Error ? error.message : "导入失败" }) } }
...
export const POST = withAdminRoute(async (request: Request) => {
  ...
  if (!id) return NextResponse.json({ success: false, error: "id 不能为空" }, { status: 400 })
  await ${className}Service.get(id)
  // Integrate the reviewed BPM/workflow adapter here; generation intentionally does not invent a workflow engine.
  domainLog.audit("${moduleName}.${camel}.submit-workflow", { targetType: "${className.toUpperCase()}", targetId: id })
  return NextResponse.json({ success: true, data: { id, status: "PENDING_WORKFLOW_INTEGRATION" } }, { status: 202 })
