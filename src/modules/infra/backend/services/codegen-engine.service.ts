...
function hasTableColumn(config: CodegenConfig, name: string): boolean {
  return configuredColumns(config).some((column) => column.name === name)
}

function persistOrderColumn(config: CodegenConfig): string {
  if (hasTableColumn(config, "update_time")) return "update_time"
  if (hasTableColumn(config, "updated_at")) return "updated_at"
  return configuredColumns(config).find((column) => column.isPrimary)?.name ?? "id"
}
...
export class CodegenEngineService {
  ...
  static generate(config: CodegenConfig): CodegenOutput[] {
    ...
    // 3. Repository (Kysely + memory) + Service + dual-mode RPC binding
    if (config.onlineRuntime?.storageKind !== "MANAGED_TABLE") outputs.push(generateRepository(config))
...
function generateRepository(config: CodegenConfig): CodegenOutput {
  const { className } = config
  const kebab = toKebab(className)
...
function scopedWhere(scope?: DynamicPersistScope) {
  ...
  if (HAS_TENANT) {
    if (!scope?.tenantId && hasRealDatabase()) throw new ApiError("FORBIDDEN", "Tenant scope is required")
    if (scope?.tenantId) predicates.push(eqColumn("tenant_id", scope.tenantId))
  }
...
function memoryMatches(row: Record<string, unknown>, scope?: DynamicPersistScope) {
  ...
  if (HAS_TENANT && scope?.tenantId && row.tenant_id !== scope.tenantId) return false
  return true
}
...
export const ${className}Repository = {
  async page(input: { page: number; pageSize: number } & Record<string, unknown>, scope?: DynamicPersistScope) {
    if (hasRealDatabase()) {
      const predicates = [scopedWhere(scope)]
    ...
    let filtered = MEMORY_STORE.filter((row) => memoryMatches(row, scope))
  ...
  async findById(id: string, scope?: DynamicPersistScope) {
    if (hasRealDatabase()) return selectDynamicById(TABLE_NAME, id, scopedWhere(scope))
    return MEMORY_STORE.find((row) => row.id === id && memoryMatches(row, scope)) ?? null
  },

  async existsByField(field: string, value: unknown, scope?: DynamicPersistScope) {
    if (hasRealDatabase()) return existsDynamicRow(TABLE_NAME, joinAnd([scopedWhere(scope), eqColumn(field, value)]))
    return MEMORY_STORE.some((row) => row[field] === value && memoryMatches(row, scope))
  },
  ...
  async create(data: Record<string, unknown>, scope?: DynamicPersistScope) {
    ...
    const row = persistable(data, {
      ...
      ...(HAS_CREATOR ? { creator: scope?.actorId ?? "" } : {}),
      ...(HAS_UPDATER ? { updater: scope?.actorId ?? "" } : {}),
      ...(HAS_DELETED ? { deleted: false } : {}),
      ...(HAS_TENANT ? { tenant_id: scope?.tenantId ?? null } : {}),
    ...
    if (hasRealDatabase()) {
      if (HAS_TENANT && !scope?.tenantId) throw new ApiError("FORBIDDEN", "Tenant scope is required")
  ...
  async update(id: string, data: Record<string, unknown>, scope?: DynamicPersistScope) {
    ...
    if (hasRealDatabase()) {
      return updateDynamicRow(TABLE_NAME, dbValues(patch), joinAnd([eqColumn("id", id), scopedWhere(scope)]))
    }
    const idx = MEMORY_STORE.findIndex((row) => row.id === id && memoryMatches(row, scope))
  ...
  async delete(id: string, scope?: DynamicPersistScope) {
    if (hasRealDatabase()) {
      if (HAS_DELETED) {
        const row = await updateDynamicRow(TABLE_NAME, dbValues({ deleted: true, ...(HAS_UPDATER ? { updater: scope?.actorId ?? "" } : {}), ...(HAS_UPDATE_TIME ? { update_time: timestamp(new Date()) } : {}), ...(HAS_UPDATED_AT ? { updated_at: timestamp(new Date()) } : {}) }), joinAnd([eqColumn("id", id), scopedWhere(scope)]))
        return Boolean(row)
      }
      return deleteDynamicRow(TABLE_NAME, joinAnd([eqColumn("id", id), scopedWhere(scope)]))
    }
    const idx = MEMORY_STORE.findIndex((row) => row.id === id && memoryMatches(row, scope))
...
function generateService(config: CodegenConfig): CodegenOutput {
  ...
  const masterUpdateSetup = masterDetailChildren.length ? `    const { children, ...parentPatch } = input as ${className}UpdateInput & { children?: Record<string, unknown> }
    const materializedChildren = children === undefined ? ((current.children as Record<string, unknown> | undefined) ?? {}) : this.materializeChildren(children, input.id)` : "    const parentPatch = input"
  ...
  static async tree(input: ${className}PageQueryInput, scope?: { tenantId?: string; actorId?: string }) {
    const page = await this.page({ ...input, page: 1, pageSize: 10000 }, scope)
  ...
  const treeCreateGuard = treeConfig ? `    const parentField = ${JSON.stringify(treeConfig.parentField)}
    ...
    if (parentId !== undefined && parentId !== null && parentId !== "" && !(await ${className}Repository.findById(String(parentId), scope))) throw new Error("父节点不存在")` : ""
  const treeUpdateGuard = treeConfig ? `    const parentField = ${JSON.stringify(treeConfig.parentField)}
    ...
    while (typeof ancestor === "string" && ancestor) {
      ...
      ancestor = (await ${className}Repository.findById(ancestor, scope))?.[parentField]
    }` : ""
  const treeDeleteGuard = treeConfig ? `    if (await ${className}Repository.existsByField(${JSON.stringify(treeConfig.parentField)}, id, scope)) throw new Error("存在子节点，不能删除；请先处理子节点")` : ""
...
type PersistScope = { tenantId?: string; actorId?: string }
const MASTER_DETAIL_CHILDREN = ${masterDetailMetadata} as const

export class ${className}Service {
  /** 分页查询 */
  static async page(input: ${className}PageQueryInput, scope?: PersistScope) {
    const result = await ${className}Repository.page(input, scope)
    domainLog.event("${moduleName}.${camel}.page", { page: input.page, total: result.total })
    return result
  }

  /** 获取详情 */
  static async get(id: string, scope?: PersistScope) {
    const item = await ${className}Repository.findById(id, scope)
    if (!item) throw new Error("${businessName}不存在")
    return item
  }

  /** 创建 */
  static async create(input: ${className}CreateInput, scope?: PersistScope) {
    const id = crypto.randomUUID()
${masterCreateSetup}
    const item: Record<string, unknown> = { id, ...parentInput${masterCreateChildren} }
${treeCreateGuard}
    const saved = await ${className}Repository.create(item, scope)
    domainLog.event("${moduleName}.${camel}.create", { id: String(saved.id) })
    domainLog.audit("${moduleName}.${camel}.create", { targetType: "${moduleName.toUpperCase()}_${className.toUpperCase()}", targetId: String(saved.id) })
    return { id: saved.id }
  }

  /** 更新 */
  static async update(input: ${className}UpdateInput, scope?: PersistScope) {
    const current = await ${className}Repository.findById(input.id, scope)
    if (!current) throw new Error("${businessName}不存在")
${masterUpdateSetup}
${treeUpdateGuard}
    const saved = await ${className}Repository.update(input.id, { ...parentPatch${masterUpdateChildren} }, scope)
    if (!saved) throw new Error("${businessName}不存在")
  ...
  /** 删除 */
  static async delete(id: string, scope?: PersistScope) {
    const current = await ${className}Repository.findById(id, scope)
    if (!current) throw new Error("${businessName}不存在")
${treeDeleteGuard}
    const removed = await ${className}Repository.delete(id, scope)
    if (!removed) throw new Error("${businessName}不存在")
...
function generateRoute(config: CodegenConfig): CodegenOutput {
  ...
  if (input.tree === "true") return NextResponse.json({ success: true, data: await ${className}Service.tree(input, scope(auth)) })` : ""
...
function scope(auth: { tenantId?: string; userId: string }) { return { tenantId: auth.tenantId, actorId: auth.userId } }

export const GET = withAdminRoute(async (request, auth) => {
  const id = new URL(request.url).searchParams.get("id")
  if (id) return NextResponse.json({ success: true, data: await ${className}Service.get(parseActionBody(${schemas}["${actionName(config, "get")}"], { id }).id, scope(auth)) })
  const input = parseActionQuery(${schemas}["${actionName(config, "page")}"], request)${treeResponse}
  return NextResponse.json({ success: true, data: await ${className}Service.page(input, scope(auth)) })
...
export const POST = withAdminRoute(async (request, auth) => {
  ...
  return NextResponse.json({ success: true, data: await ${className}Service.create(input, scope(auth)) }, { status: 201 })
...
export const PUT = withAdminRoute(async (request, auth) => {
  ...
  return NextResponse.json({ success: true, data: await ${className}Service.update(input, scope(auth)) })
...
export const DELETE = withAdminRoute(async (request, auth) => {
  ...
  return NextResponse.json({ success: true, data: await ${className}Service.delete(input.id, scope(auth)) })
...
export const GET = withAdminRoute(async (request, auth) => {
  const page = await ${className}Service.page(parseActionQuery(${schemas}["${actionName(config, "page")}"], request), scope(auth))
...
export const POST = withAdminRoute(async (request, auth) => {
  ...
  for (let index = 0; index < body.rows.length; index++) { try { await ${className}Service.create(parseActionBody(${schemas}["${actionName(config, "create")}"], body.rows[index]), scope(auth)); imported++ } catch (error) { errors.push({ index, error: error instanceof Error ? error.message : "导入失败" }) } }
...
export const POST = withAdminRoute(async (request, auth) => {
  ...
  await ${className}Service.get(input.id, scope(auth))
