...
export default function InfraDbConfigsModulePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const response = await dataSourceConfigApi.page({ page, pageSize: 20, keyword: keyword || undefined })
  ...
  async function remove(item: DataSourceConfig) {
    ...
    const response = await dataSourceConfigApi.remove(item.id)
  ...
  async function test(item: DataSourceConfig) {
    ...
    setTesting(item.id)
    const response = await dataSourceConfigApi.test({ id: item.id })
    setTesting(null)
