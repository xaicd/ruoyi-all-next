...
import { WmsInventoryHistoryApi } from "../api/wms-inventory-history.api"
import { WmsInventoryHistoryForm } from "../components/WmsInventoryHistoryForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsInventoryHistoryVO, WmsInventoryHistoryPageQuery } from "@/modules/wms/backend/types/wms-inventory-history.types"

export function WmsInventoryHistoryListPage() {
  const [data, setData] = useState<WmsInventoryHistoryVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsInventoryHistoryVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsInventoryHistoryApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsInventoryHistoryVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsInventoryHistoryApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">库存流水管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">库存流水列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">关联单号:</label>
          <input
            type="text"
            value={searchForm.ref_order_no ?? ""}
            onChange={(e) => setSearchForm((prev) => ({ ...prev, ref_order_no: e.target.value }))}
            placeholder="搜索关联单号"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">仓库ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">物料ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">变动类型</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">变动数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">变动前数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">变动后数量</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.change_type ?? "-")}>{String(item.change_type ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.qty_change ?? "-")}>{String(item.qty_change ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.before_qty ?? "-")}>{String(item.before_qty ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.after_qty ?? "-")}>{String(item.after_qty ?? "-")}</td>
