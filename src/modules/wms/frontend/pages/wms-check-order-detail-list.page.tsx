...
import { WmsCheckOrderDetailForm } from "../components/WmsCheckOrderDetailForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsCheckOrderDetailVO, WmsCheckOrderDetailPageQuery } from "@/modules/wms/backend/types/wms-check-order-detail.types"

export function WmsCheckOrderDetailListPage() {
  const [data, setData] = useState<WmsCheckOrderDetailVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsCheckOrderDetailVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsCheckOrderDetailApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsCheckOrderDetailVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsCheckOrderDetailApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">盘点明细管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">盘点明细列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">关联盘点单ID:</label>
          <input
            type="text"
            value={searchForm.check_order_id ?? ""}
            onChange={(e) => setSearchForm((prev) => ({ ...prev, check_order_id: e.target.value }))}
            placeholder="搜索关联盘点单ID"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">关联盘点单ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">物料ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">账面数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">实盘数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">差异数量</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.check_order_id ?? "-")}>{String(item.check_order_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.item_id ?? "-")}>{String(item.item_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.system_qty ?? "-")}>{String(item.system_qty ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.check_qty ?? "-")}>{String(item.check_qty ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.diff_qty ?? "-")}>{String(item.diff_qty ?? "-")}</td>
