...
import { WmsReceiptOrderApi } from "../api/wms-receipt-order.api"
import { WmsReceiptOrderForm } from "../components/WmsReceiptOrderForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsReceiptOrderVO, WmsReceiptOrderPageQuery } from "@/modules/wms/backend/types/wms-receipt-order.types"

export function WmsReceiptOrderListPage() {
  const [data, setData] = useState<WmsReceiptOrderVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsReceiptOrderVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsReceiptOrderApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsReceiptOrderVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsReceiptOrderApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">入库单管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">入库单列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">入库单号:</label>
          <input
            type="text"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">入库单号</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">入库类型</th>
                ...
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">总入库数量</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.receipt_type ?? "-")}>{String(item.receipt_type ?? "-")}</td>
