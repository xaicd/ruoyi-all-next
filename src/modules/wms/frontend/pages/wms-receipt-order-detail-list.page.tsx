...
import { WmsReceiptOrderDetailApi } from "../api/wms-receipt-order-detail.api"
import { WmsReceiptOrderDetailForm } from "../components/WmsReceiptOrderDetailForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsReceiptOrderDetailVO, WmsReceiptOrderDetailPageQuery } from "@/modules/wms/backend/types/wms-receipt-order-detail.types"

export function WmsReceiptOrderDetailListPage() {
  const [data, setData] = useState<WmsReceiptOrderDetailVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsReceiptOrderDetailVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsReceiptOrderDetailApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsReceiptOrderDetailVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsReceiptOrderDetailApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">入库明细管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">入库明细列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">关联入库单ID:</label>
          <input
            type="text"
            value={searchForm.receipt_order_id ?? ""}
            onChange={(e) => setSearchForm((prev) => ({ ...prev, receipt_order_id: e.target.value }))}
            placeholder="搜索关联入库单ID"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">关联入库单ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">物料ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">计划入库数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">实际入库数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">批次号</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.receipt_order_id ?? "-")}>{String(item.receipt_order_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.item_id ?? "-")}>{String(item.item_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.plan_qty ?? "-")}>{String(item.plan_qty ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.real_qty ?? "-")}>{String(item.real_qty ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.batch_no ?? "-")}>{String(item.batch_no ?? "-")}</td>
