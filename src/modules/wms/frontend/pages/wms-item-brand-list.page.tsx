...
import { WmsItemBrandApi } from "../api/wms-item-brand.api"
import { WmsItemBrandForm } from "../components/WmsItemBrandForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsItemBrandVO, WmsItemBrandPageQuery } from "@/modules/wms/backend/types/wms-item-brand.types"

export function WmsItemBrandListPage() {
  const [data, setData] = useState<WmsItemBrandVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsItemBrandVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsItemBrandApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsItemBrandVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsItemBrandApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">物料品牌管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">物料品牌列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">品牌名称:</label>
          <input
            type="text"
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">品牌编码:</label>
          <input
            type="text"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">品牌名称</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">品牌编码</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">品牌LOGO</th>
