...
import { WmsCheckOrderForm } from "../components/WmsCheckOrderForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsCheckOrderVO, WmsCheckOrderPageQuery } from "@/modules/wms/backend/types/wms-check-order.types"

export function WmsCheckOrderListPage() {
  const [data, setData] = useState<WmsCheckOrderVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsCheckOrderVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsCheckOrderApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsCheckOrderVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsCheckOrderApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">盘点单管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">盘点单列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">盘点单号:</label>
          <input
            type="text"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">盘点单号</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">盘点仓库ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">盘点类型</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.check_type ?? "-")}>{String(item.check_type ?? "-")}</td>
