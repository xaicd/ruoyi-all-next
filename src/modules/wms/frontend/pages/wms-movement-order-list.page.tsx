...
import { WmsMovementOrderApi } from "../api/wms-movement-order.api"
import { WmsMovementOrderForm } from "../components/WmsMovementOrderForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsMovementOrderVO, WmsMovementOrderPageQuery } from "@/modules/wms/backend/types/wms-movement-order.types"

export function WmsMovementOrderListPage() {
  const [data, setData] = useState<WmsMovementOrderVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsMovementOrderVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsMovementOrderApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsMovementOrderVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsMovementOrderApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">移库单管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">移库单列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">移库单号:</label>
          <input
            type="text"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">移库单号</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">源仓库ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">目标仓库ID</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.from_warehouse_id ?? "-")}>{String(item.from_warehouse_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.to_warehouse_id ?? "-")}>{String(item.to_warehouse_id ?? "-")}</td>
