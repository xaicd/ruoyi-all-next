...
import { WmsMovementOrderDetailForm } from "../components/WmsMovementOrderDetailForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsMovementOrderDetailVO, WmsMovementOrderDetailPageQuery } from "@/modules/wms/backend/types/wms-movement-order-detail.types"

export function WmsMovementOrderDetailListPage() {
  const [data, setData] = useState<WmsMovementOrderDetailVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsMovementOrderDetailVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsMovementOrderDetailApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsMovementOrderDetailVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsMovementOrderDetailApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">移库明细管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">移库明细列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">关联移库单ID:</label>
          <input
            type="text"
            value={searchForm.movement_order_id ?? ""}
            onChange={(e) => setSearchForm((prev) => ({ ...prev, movement_order_id: e.target.value }))}
            placeholder="搜索关联移库单ID"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">关联移库单ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">物料ID</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">移库数量</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-xs text-slate-400">加载中...</td>
                </tr>
              ) : data.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-xs text-slate-400">暂无数据</td>
                </tr>
              ) : (
                data.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.movement_order_id ?? "-")}>{String(item.movement_order_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.item_id ?? "-")}>{String(item.item_id ?? "-")}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.qty ?? "-")}>{String(item.qty ?? "-")}</td>
