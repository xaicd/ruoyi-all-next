...
import { WmsInventoryForm } from "../components/WmsInventoryForm"
import { Pagination } from "@/modules/shared/frontend/components/pagination"
import type { WmsInventoryVO, WmsInventoryPageQuery } from "@/modules/wms/backend/types/wms-inventory.types"

export function WmsInventoryListPage() {
  const [data, setData] = useState<WmsInventoryVO[]>([])
  ...
  const [editItem, setEditItem] = useState<WmsInventoryVO | null>(null)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await WmsInventoryApi.page({ page, pageSize, ...searchForm })
      setData(res.items || [])
  ...
  const handleEdit = (item: WmsInventoryVO) => {
    setEditItem(item)
    setFormOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await WmsInventoryApi.delete(id)
      fetchData()
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">实时库存管理</h1>
          <p className="text-xs text-slate-500 mt-0.5">实时库存列表与配置管理</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-slate-500 whitespace-nowrap">仓库ID:</label>
          <input
            type="text"
            value={searchForm.warehouse_id ?? ""}
            onChange={(e) => setSearchForm((prev) => ({ ...prev, warehouse_id: e.target.value }))}
            placeholder="搜索仓库ID"
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/80">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">现存数量</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">锁定数量</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {loading ? (
                ...
                data.map((item) => (
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-700 max-w-[200px] truncate" title={String(item.locked_qty ?? "-")}>{String(item.locked_qty ?? "-")}</td>
