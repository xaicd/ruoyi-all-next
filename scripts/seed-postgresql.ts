...
async function main() {
  ...
  try {
    ...
    await client.query(`INSERT INTO system_tenant (id, name, contact_name, contact_phone, domain, package_id, status, expire_time, account_count, created_at, updated_at, deleted) VALUES ('1','默认租户','管理员','13800000001',NULL,'111','ACTIVE','2030-12-31T23:59:59.000Z',999,'2026-01-01T00:00:00.000Z','2026-01-01T00:00:00.000Z',false) ON CONFLICT (id) DO UPDATE SET package_id = EXCLUDED.package_id, updated_at = EXCLUDED.updated_at, deleted = false`)
