  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">盘点单 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">盘点单号: {{ item.order_no || '-' }}</text>
        <text class="card-item">盘点仓库ID: {{ item.warehouse_id || '-' }}</text>
        <text class="card-item">盘点类型: {{ item.check_type || '-' }}</text>
        <text class="card-item">单据状态: {{ item.status || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsCheckOrderList, type WmsCheckOrderItem } from '../api/wms-check-order.api'

const items = ref<WmsCheckOrderItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsCheckOrderList()
