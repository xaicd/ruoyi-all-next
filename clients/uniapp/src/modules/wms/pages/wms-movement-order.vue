  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">移库单 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">移库单号: {{ item.order_no || '-' }}</text>
        <text class="card-item">源仓库ID: {{ item.from_warehouse_id || '-' }}</text>
        <text class="card-item">目标仓库ID: {{ item.to_warehouse_id || '-' }}</text>
        <text class="card-item">移库数量: {{ item.total_qty || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsMovementOrderList, type WmsMovementOrderItem } from '../api/wms-movement-order.api'

const items = ref<WmsMovementOrderItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsMovementOrderList()
