  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">物料品牌 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">品牌名称: {{ item.name || '-' }}</text>
        <text class="card-item">品牌编码: {{ item.code || '-' }}</text>
        <text class="card-item">品牌LOGO: {{ item.logo || '-' }}</text>
        <text class="card-item">状态: {{ item.status || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsItemBrandList, type WmsItemBrandItem } from '../api/wms-item-brand.api'

const items = ref<WmsItemBrandItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsItemBrandList()
