  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">实时库存 #{{ item.id.slice(0, 8) }}</text>
        ...
        <text class="card-item">现存数量: {{ item.qty || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsInventoryList, type WmsInventoryItem } from '../api/wms-inventory.api'

const items = ref<WmsInventoryItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsInventoryList()
