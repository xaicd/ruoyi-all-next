  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">物料分类 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">上级分类ID: {{ item.parent_id || '-' }}</text>
        <text class="card-item">分类名称: {{ item.name || '-' }}</text>
        <text class="card-item">分类编码: {{ item.code || '-' }}</text>
        <text class="card-item">显示排序: {{ item.sort || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsItemCategoryList, type WmsItemCategoryItem } from '../api/wms-item-category.api'

const items = ref<WmsItemCategoryItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsItemCategoryList()
