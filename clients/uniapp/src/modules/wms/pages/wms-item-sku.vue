  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">物料SKU #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">物料ID: {{ item.item_id || '-' }}</text>
        <text class="card-item">SKU编码: {{ item.sku_code || '-' }}</text>
        <text class="card-item">规格名称: {{ item.sku_name || '-' }}</text>
        <text class="card-item">条形码: {{ item.barcode || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsItemSkuList, type WmsItemSkuItem } from '../api/wms-item-sku.api'

const items = ref<WmsItemSkuItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsItemSkuList()
