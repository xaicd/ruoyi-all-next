  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">货主管理 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">货主编码: {{ item.code || '-' }}</text>
        <text class="card-item">货主名称: {{ item.name || '-' }}</text>
        <text class="card-item">联系人: {{ item.contact_name || '-' }}</text>
        <text class="card-item">联系电话: {{ item.contact_phone || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsMerchantList, type WmsMerchantItem } from '../api/wms-merchant.api'

const items = ref<WmsMerchantItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsMerchantList()
