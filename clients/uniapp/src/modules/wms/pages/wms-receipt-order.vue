  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">入库单 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">入库单号: {{ item.order_no || '-' }}</text>
        <text class="card-item">入库类型: {{ item.receipt_type || '-' }}</text>
        <text class="card-item">目标仓库ID: {{ item.warehouse_id || '-' }}</text>
...
import { fetchWmsReceiptOrderList, type WmsReceiptOrderItem } from '../api/wms-receipt-order.api'

const items = ref<WmsReceiptOrderItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsReceiptOrderList()
