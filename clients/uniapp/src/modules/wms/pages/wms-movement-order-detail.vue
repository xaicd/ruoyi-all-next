  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">移库明细 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">关联移库单ID: {{ item.movement_order_id || '-' }}</text>
        <text class="card-item">物料ID: {{ item.item_id || '-' }}</text>
        <text class="card-item">移库数量: {{ item.qty || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsMovementOrderDetailList, type WmsMovementOrderDetailItem } from '../api/wms-movement-order-detail.api'

const items = ref<WmsMovementOrderDetailItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsMovementOrderDetailList()
