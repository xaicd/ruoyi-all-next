  ...
  <view class="container">
    ...
    <view v-else class="list">
      <view v-for="item in items" :key="item.id" class="card">
        <text class="card-title">出库明细 #{{ item.id.slice(0, 8) }}</text>
        <text class="card-item">关联出库单ID: {{ item.shipment_order_id || '-' }}</text>
        <text class="card-item">物料ID: {{ item.item_id || '-' }}</text>
        <text class="card-item">计划出库数量: {{ item.plan_qty || '-' }}</text>
        <text class="card-item">实际出库数量: {{ item.real_qty || '-' }}</text>
      </view>
    </view>
  </view>
</template>
...
import { fetchWmsShipmentOrderDetailList, type WmsShipmentOrderDetailItem } from '../api/wms-shipment-order-detail.api'

const items = ref<WmsShipmentOrderDetailItem[]>([])
const loading = ref(false)

onMounted(async () => {
  loading.value = true
  try {
    const res = await fetchWmsShipmentOrderDetailList()
