...
| D28 | DONE | 业务项目身份初始化底座 | 新项目只改 `project-profile.json`、Logo 与 `.env.local` 默认账号；seed 写入平台名/租户/套餐展示名 |
| D29 | DONE | 多端能力标准 | PC / H5 / uni-app / Flutter / 桌面客户端共用 admin/app/open 面；各端独立包且按域名分目录；H5 等工程尚未交付 |
